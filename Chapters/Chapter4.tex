\chapter{An Exact Kernel Equivalence for Finite Classification Models} % Main chapter title
...
%\newcommand{\theHalgorithm}{\arabic{algorithm}}os
...
The resulting paper which is presented here exactly as accepted to the
...
\end{restatable}
%See Appendix~\ref{proof:ekmr} for full proof.
...
% (See Appendix section ~\ref{proof:ekmr}).
...
% show kernel values or mean prediction with variances from GP
    \begin{figure}[h]
        ...
        \caption{(left) Kernel predicted probabilities measured on a grid around the training set for our 2D problem. Bright yellow means high kernel value. (right) Monte-Carlo estimated standard deviation based on gram matrices generated using our kernel for the same grid as the kernel values. Yellow means high standard deviation, blue means low standard deviation.}
        \label{fig:cov}
    \end{figure}
...
\caption{This plot shows a linear interpolation $w(t) = w_0 + t(w_{1} - w_0)$ of model parameters $w$ for a convolutional neural network $f_w$ from their starting random state $w_0$ to their ending trained state $w_1$. The hatched purple line shows the dot product of the sum of the  gradient over the training data $X$, $\langle \nabla_w f_{w(t)}(X), (w_1 - w_0)/|w_1 - w_0| \rangle$. The other lines indicate accuracy (blue), L2 Regularization (green increasing), total loss (red decreasing), and cross-entropy loss (orange, covered exactly by total loss showing that cross-entropy dominates the total loss for this regularization weight). Main takeaway is that model training gradients get more aligned with a the vector difference between the start and end point of training as the model trains. Also, that vector is very stable past a certain distance away from the initial training weights.}
\label{fig:grad}
\end{figure}
